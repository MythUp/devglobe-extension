...
The skill hooks into Codex lifecycle events (`SessionStart`, `UserPromptSubmit`, `Stop`) and sends a heartbeat to DevGlobe at most once per minute. It automatically detects:
...
| `$devglobe check` | Verify the installation (config, hooks, feature flag) |
